...
def open_first_page(num=None):
    ...
    if entered_password == password:
        try:
            subprocess.Popen(["python", "PMS.py"])
        ...
        messagebox.showerror("Invalid Password", "Maybe only authentic Gajul's possess the secret code!")
...
def change_password():
    ...
    old_password = simpledialog.askstring("Change Password", "Enter Old Password:", parent=root)
    if old_password != password:
        messagebox.showerror("Invalid Password", "Old password is incorrect.")
        return
    
    # Prompt the user for the new password
    new_password = simpledialog.askstring("Change Password", "Enter new password:", parent=root)
    if new_password:
...
# Add Password label 
password_label = tk.Label(root, text="Only true Gajul's holds the key to the word.", font=("Garamond, 16"))
password_label.pack()
...
next_page_button = tk.Button(root, text="Let's Start!", command=open_first_page, font=("Garamond", 20, "bold"), height=2, width=12,cursor="hand2")
...
change_password_button = tb.Button(root, text="Change Password", bootstyle=("link"), command=change_password)
...
def forgot_password():
    ...
    try:
        ...
        if answer.lower() == "suvarna":
            ...
            messagebox.showinfo("Forgot Password", f"Your current password is: {current_password}")
        else:
...
forgot_password_button = tb.Button(root, text="Forgot Password", bootstyle=("link"), command=forgot_password)
forgot_password_button.pack()

def exit_window():
    root.quit()

exit_button = tk.Button(root, text='Exit', command=exit_window, font=('Garamond', 15, 'bold'), height=1, width=6, cursor="hand2")
