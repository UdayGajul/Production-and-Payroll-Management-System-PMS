...
class PMS:
    def __init__(self, root):
        ...
        try:
            ...
            # print("Database 'production' created successfully.")
...
# =====================================================================================================================            
            if number == 1:
                ...
                # 👇👇👇 here you must write code for home button/frame 👇👇👇 
                ...
                def generate_salary_data():
                    ...
                    if worker_id_sal.get() == "-Select-":
                        ...
                        try:
                            ...
                            if total_salary == 0:
                                ...
                                # messagebox.askyesno("Save Salary", "Do you want to save the salary data?")
                    ...
                    # try:
                    #     # Execute SQL query to calculate the salary
                    #     sql_salary = ("""SELECT P.Date, P.Day, P.Loom_No, P.Worker_Id, CONCAT(W.Name, ' ', W.Surname) AS Worker_Name, 
                    #                     P.Production, CONCAT((P.Production * PR.Price), 2) AS Salary 
                    #                     FROM Production P
                    #                     JOIN Workers W ON P.Worker_Id = W.Worker_Id
                    #                     JOIN Looms L ON P.Loom_No = L.Loom_No
                    #                     JOIN Product PR ON L.Product_Id = PR.Product_Id
                    ...
                    #     total_salary = sum(row[6] for row in rows)  # Sum up the salaries
                    #     self.mydb.commit()
                    #     # Display the total salary in a messagebox
                    #     messagebox.showinfo("Total Salary", f"Total Salary: {total_salary:.2f}")
                    # except mysql.connector.Error as err:
                    #     messagebox.showerror("Oops!", str(err))

                        
                    # # Ask user if they want to save the salary data
                    # # if messagebox.askyesno("Save Salary", "Do you want to save the salary data?"):
                    # #     # Execute SQL query to save salary data to the Salaries table
                    # #     for row in rows:
                    # #         self.mycursor.execute("""
                    # #             INSERT INTO Salaries (Date, Worker_Id, Salary)
                    # #             VALUES (%s, %s, %s)
                    # #         """, (row[0], row[3], row[6]))
                    # #     self.mydb.commit()
                ...
                # self.mycursor.execute("SELECT DISTINCT Date FROM Production ORDER BY Date DESC")
                # dates = self.mycursor.fetchall()
                # date_list = ["-Select-"] + [date[0] for date in dates]

                # from_date_combobox = Combobox(
                #     salary_frame,
                #     values=date_list,
                #     font=('Garamond', 12),
                #     state="readonly",
                #     justify=CENTER,
                #     textvariable=from_date
                # )
                # from_date_combobox.current(0)
                # from_date_combobox.place(x=540, y=270, height=30, width=180)
                ...
                # to_date_combobox = Combobox(
                #     salary_frame,
                #     values=date_list,
                #     font=('Garamond', 12),
                #     state="readonly",
                #     justify=CENTER,
                #     textvariable=to_date
                # )
                # to_date_combobox.current(0)
                # to_date_combobox.place(x=540, y=340, height=30, width=180)
...
# =====================================================================================================================
                ...
                # def get_product_data(eve):
                #     # try:
                #         r = product_table.focus()
                #         content = (product_table.item(r))
                        
                #         row = content['values']
                        
                #         # product_id_cb.set(row[0])
                #         product_name_cb.set(str(row[1]).strip())
                #         product_size_cb.set(str(row[2]).strip())
                #         product_price_var.set(row[3])
                #     # except IndexError as ie:
                #     #     messagebox.showerror("Oops!", str(ie))
                ...
                def fill_product_details(eve):
                    try:
                        ...
                        if set_product:
                            ...
                            # print(selected_product_name, selected_product_size, selected_product_price)
                ...
                # product_table.bind("<ButtonRelease-1>", get_product_data)
...
# =====================================================================================================================
                ...
                def update_looms_data():
                    ...
                    if loom_no_cb.get() == "-Select-":
                        ...
                        #     sql_check = "SELECT Size FROM Product"
                        #     self.mycursor.execute(sql_check, ())
                        #     existing_size = self.mycursor.fetchall()
                        #     print(existing_size)
                            
                        #     if (int(product_size_val),) in existing_size:
                        #         if product_size_val == "3060":
                                    
                        #             sql_update_looms = "UPDATE Looms SET Product_Id = 101 WHERE Loom_No = %s"
                        #             self.mycursor.execute(sql_update_looms,(loom_no_cb_val,))
                        #             self.mydb.commit()
                        #             messagebox.showinfo("Success", "Loom data updated successfully.")
                        #             loom_no_cb.set("-Select-")
                        #             product_size.set("-Select-")
                                    
                        #         elif product_size_val == "2754":
                                    
                        #             sql_update_looms1 = "UPDATE Looms SET Product_Id = 102 WHERE Loom_No = %s"
                        #             self.mycursor.execute(sql_update_looms1, (loom_no_cb_val,))
                        #             self.mydb.commit()
                        #             messagebox.showinfo("Success", "Loom data updated successfully.")
                        #             loom_no_cb.set("-Select-")
                        #             product_size.set("-Select-")
                                    
                        #         elif product_size_val == "1421":
                                    
                        #             sql_update_looms2 = "UPDATE Looms SET Product_Id = 103 WHERE Loom_No = %s"
                        #             self.mycursor.execute(sql_update_looms2, (loom_no_cb_val,))
                        #             self.mydb.commit()
                        #             messagebox.showinfo("Success", "Loom data updated successfully.")
                        #             loom_no_cb.set("-Select-")
                        #             product_size.set("-Select-")
                                    
                        #         elif product_size_val == "1218":
                        #             sql_update_looms3 = "UPDATE Looms SET Product_Id = 104 WHERE Loom_No = %s"
                        #             self.mycursor.execute(sql_update_looms3, (loom_no_cb_val,))
                        #             self.mydb.commit()
                        #             messagebox.showinfo("Success", "Loom data updated successfully.")
                        #             loom_no_cb.set("-Select-")
                        #             product_size.set("-Select-")
                        ...
                        #     else:
                        #         messagebox.showerror("Oops", "Product is not present, try adding it first.")
...
# =====================================================================================================================  
                ...
                # x=750, y=200, height=30, width=100
                ...
                # x=750, y=270, height=30, width=100
                ...
                # x=750, y=340, height=30, width=100
                ...
                # x=540, y=410, height=30, width=100
                
                # # === Instruction LabelFrame for beginners ===
                # instruction_label_frame = LabelFrame(
                #     workers_frame,
                #     text=' Instructions ',
                #     font=('Garamond', 17, 'bold'),
                #     bd=2,
                #     relief=RIDGE
                # )
                # instruction_label_frame.place(x=950, y=185, height=190, width=600)
                
                # # === Instructions Labels inside instruction_label_frame ===
                # instruction_label_1 = Label(
                #     instruction_label_frame,
                #     text='1) You dont need to worry about the date and day to be entered.',
                #     font=('Garamond',)
                # ).place(x=0, y=0)
                # instruction_label_2 = Label(
                #     instruction_label_frame,
                #     text='2) To permanently remove a worker\'s data: Enter their ID and confirm your action by\npressing the Delete button. Please note that this action is irreversible.',
                #     font=('Garamond',)
                # ).place(x=0, y=20)
                # instruction_label_3 = Label(
                #     instruction_label_frame,
                #     text='3) You can search for the worker using their name or surname.',
                #     font=('Garamond',)
                # ).place(x=0, y=60)
...
# =====================================================================================================================
                ...
                #     loom_no_str = loom_no.get().strip()
                #     if production.get() == "":
                #         messagebox.showerror("Oops", "You missed a spot. Please fill in the required textbox.")
                #     elif loom_no.get() == "-Select-":
                #         messagebox.showerror("Oops!", "Try another option for Loom No.")
                #     elif worker_id_cb.get() == "-Select-":
                #         messagebox.showerror("Oops!", "Try another option for Worker ID.")
                #   elif p_id_cb.get() == "-Production ID-":
                #        messagebox.showerror("Oops!", "Try another option for Production ID.")
                #     else:
                #         try:
                #             sql_check_production = "SELECT Date FROM production WHERE Loom_No = %s ORDER BY Date DESC"
                #             self.mycursor.execute(sql_check_production, (loom_no_str, ))
                            
                #             today_date = self.mycursor.fetchall()
                #             current_date = str(datetime.today().date())
                            
                #             if today_date:
                #                 today_date_str = today_date[0][0].strftime('%Y-%m-%d')

                #                 if today_date_str != current_date:
                #                     messagebox.showerror("Oops!", f"No existing data found for Loom No {loom_no_str}. Please insert data first.")
                #                 elif today_date_str == current_date:
                #                     loom_no_var, worker_id_var, production_var = gather_production_data()
                #                     sql_update_production = "UPDATE Production SET Worker_Id = %s, Production = %s WHERE Loom_No = %s AND Date = DATE(%s)"
                #                     values = (worker_id_var, production_var, loom_no_str, current_date)
                #                     self.mycursor.execute(sql_update_production, values)
                #                     self.mydb.commit()
                #                     messagebox.showinfo("Success", f"Loom No {loom_no_str}'s data has been updated successfully.")
                #             else:
                #                 messagebox.showerror("Oops!", "No existing data found for the given Loom No.")
                ...
                def show_production_data():
                    # try:
                        ...
                        rows = self.mycursor.fetchall()
                        # rows.reverse() # Reverse the order of the rows
                    ...
                    # except mysql.connector.Error as err:
                    #     messagebox.showerror("Oops!", "Error displaying the data try again later.")
                ...
                def get_production_data(event1):
                    ...
                    # else:
                    #     print("Row is empty or does not contain expected values:", row)
                ...
                #     state="readonly",
                #     justify=CENTER,
                #     textvariable=p_id_cb
                # )
                # pid_combobox.current(0)
                # pid_combobox.place(x=750, y=200, height=30, width=140)
...
# =====================================================================================================================
                ...
                def display_report():
                    ...
                    if var.get() == 1:
                        try:
                            ...
                            fig, ax = plt.subplots(figsize=(12.7,6.8))  # Create a figure and axes within report_frame
                        ...
                        try:
                            ...
                            # print(salaries_values)
                            
                            fig1, ax1 = plt.subplots(figsize=(12.7,6.8))
                ...
                #     font=('Garamond', 15),
                #     cursor="hand2",
                #     command=destroy_canvas
                # )
                # destroy_button.place(x=700, y=15, height=30, width=150)
    ...
    def exit_the_window(self):
        # Close the Data_Entry.py window logic
        # First closing the database connection
        if hasattr(self, 'mydb'):
            self.mydb.close()
            # print('db closed')
