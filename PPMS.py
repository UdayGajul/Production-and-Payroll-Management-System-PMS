...
def parse_date_entry(date_string):
    """
    Universal date parser that handles multiple date formats from DateEntry widget.
    ...
    date_formats = [
        "%m/%d/%Y",  # 02/16/2026
        "%m/%d/%y",  # 02/16/26
        "%#m/%#d/%Y",  # 2/16/2026 (Windows)
        "%#m/%#d/%y",  # 2/16/26 (Windows)
        "%-m/%-d/%Y",  # 2/16/2026 (Unix/Linux)
        "%-m/%-d/%y",  # 2/16/26 (Unix/Linux)
    ]
    ...
    # If none of the formats work, raise a clear error
    raise ValueError(
        f"Unable to parse date: '{date_string}'. Expected format: MM/DD/YYYY or MM/DD/YY"
    )


def create_resizable_chart(parent_frame, fig, chart_y_position=60):
    """
    Creates a resizable chart that automatically adjusts to parent frame size.

    Args:
        parent_frame: The tkinter frame containing the chart
        fig: The matplotlib figure object
        chart_y_position: Y-position where chart starts (default 60 to avoid buttons)

    Returns:
        canvas: The FigureCanvasTkAgg canvas object
    """
    # Create canvas
    ...
    def resize_chart(event=None):
        """Dynamically resize chart based on parent frame dimensions"""
        try:
            # Get parent frame dimensions
            frame_width = parent_frame.winfo_width()
            frame_height = parent_frame.winfo_height()

            # Calculate available space (subtract Y-position offset and padding)
            available_width = frame_width - 20  # 20px padding
            available_height = (
                frame_height - chart_y_position - 20
            )  # Space below buttons

            # Convert pixels to inches (assuming 100 DPI)
            width_inches = available_width / 100
            height_inches = available_height / 100

            # Set minimum sizes to prevent too small charts
            width_inches = max(width_inches, 10)
            height_inches = max(height_inches, 6)

            # Resize the figure
            fig.set_size_inches(width_inches, height_inches, forward=True)
    ...
    parent_frame.bind("<Configure>", resize_chart)

    # Initial resize
    parent_frame.after(100, resize_chart)
...
class PMS:
    def __init__(self, root):
        ...
        self.root.geometry(
            "1440x900"
        )  # This could be useful after Restoring Down the window
        ...
        # --==--==NEW:--==--==
        self.icon_title = PhotoImage(file=resource_path("images/favicon-3.png"))
        ...
        )  # Placing the title label inside the window
        ...
        logout_button = Button(
            ...
            x=1750, y=20, height=50, width=150
        )  # Placing the logout button inside the window

        # ===Clock===
        self.clock_label = Label(
            self.root, text="Clock", font=("Garamond", 15)  # Initialize with empty text
        )
        self.clock_label.place(
            x=0, y=70, relwidth=1, height=30
        )  # Placing the title label inside the window
        ...
        self.mydb = mysql.connector.connect(
            host="localhost", user="root", password="admin", database="production"
        )
        ...
        def switch_frames(number):
            ...
            if number == 1:
                ...
                def generate_salary_data():
                    ...
                    # --==--== new code --==--==
                    from_date = my_date.entry.get()
                    selected_from_date = parse_date_entry(from_date).strftime(
                        "%Y-%m-%d"
                    )

                    to_date = my_date_1.entry.get()
                    selected_to_date = parse_date_entry(to_date).strftime("%Y-%m-%d")
                        ...
                        try:
                            ...
                            if total_salary == 0:
                                messagebox.showerror(
                                    "Oops!",
                                    "No production data found for the selected worker during the specified date range.",
                                )
                            else:
                                save_salary = messagebox.askyesno(
                                    "Total Salary",
                                    f"Total Salary: INR {total_salary:.2f}\nDo you want to save the salary data?",
                                )
                                ...
                                if save_salary == True:
                                    ...
                                    try:
                                        ...
                                        self.mycursor.execute(
                                            "SELECT CONCAT(Name, ' ', Surname) AS Full_Name FROM Workers WHERE Worker_Id = %s",
                                            (selected_worker_id,),
                                        )
                                        ...
                                        messagebox.showinfo(
                                            "Salary Saved",
                                            f"Salary information for {full_name} saved successfully.",
                                        )
                ...
                home_title = Label(salary_frame, text="Salary", font=("Garamond", 20))
                home_title.place(x=50, y=120, width=1200)

                home_worker_label = Label(
                    salary_frame, text="Worker ID", font=("Garamond", 15)
                )
                home_worker_label.place(x=350, y=200)

                self.mycursor.execute(
                    "SELECT Worker_Id FROM workers"
                )  # Fetch the Worker_Id values from the workers table
                worker_ids1 = (
                    self.mycursor.fetchall()
                )  # Fetch all rows from the query result
                worker_id_list = ["-Select-"] + [
                    worker[0] for worker in worker_ids1
                ]  # Extract Worker_Id values from the result
                ...
                from_date_label = Label(
                    salary_frame, text="From Date", font=("Garamond", 15)
                )
                from_date_label.place(x=350, y=270)

                my_date = tb.DateEntry(
                    salary_frame, bootstyle="danger", dateformat="%m/%d/%y"
                )
                my_date.place(x=540, y=270)

                to_date_label = Label(
                    salary_frame, text="To Date", font=("Garamond", 15)
                )
                ...
                p_sb_y2 = Scrollbar(worker_table_frame3, orient=VERTICAL)
                p_sb_y2.pack(side=RIGHT, fill=Y)
                p_sb_x2 = Scrollbar(worker_table_frame3, orient=HORIZONTAL)
                ...
                p_sb_y9 = Scrollbar(home_table_frame, orient=VERTICAL)
                p_sb_y9.pack(side=RIGHT, fill=Y)
                p_sb_x9 = Scrollbar(home_table_frame, orient=HORIZONTAL)
                ...
                )  # Pack the salary_frame into the main_frame
            ...
            # =====================================================================================================================
                ...
                def save_product():
                    ...
                    if product_price_str == "":
                        ...
                        not product_name_var.get().replace(" ", "").isalpha()
                        and not " " in product_name_var.get()
                    ):
                        messagebox.showerror(
                            "Oops!", "Product name should not contain numbers."
                        )

                    elif not product_size_var.get().isdigit():
                        messagebox.showerror(
                            "Oops!", "Product size should only contain numbers."
                        )
                    ...
                    #     messagebox.showerror("Oops!", "Product size should be one of: 4070, 3060, 3056, 2754, 2030, 1421, 1218, 1212.")
                        ...
                        messagebox.showerror(
                            "Oops!", "Product Price accepts only numeric values."
                        )
                ...
                def update_products():
                    ...
                    if (
                        product_id_str == ""
                        or product_name_cb.get() == ""
                        or product_size_cb.get() == ""
                        or product_price_var.get() == ""
                    ):
                        messagebox.showerror(
                            "Oops!",
                            "You missed a spot. Please fill in the required textbox(s).",
                        )
                        ...
                        try:
                            ...
                            if existing_product is None:
                                messagebox.showerror(
                                    "Oops!",
                                    f"Product with ID {product_id_str} does not exist.",
                                )

                            else:
                                p_id_val, p_nm_val, p_si_val, p_pr_val = (
                                    gather_product_data()
                                )
                ...
                def enable_cb_product():
                    ...
                    product_name_list = ["-Select-"] + [
                        name[0] for name in product_names
                    ]
                    ...
                    product_size_list = ["-Select-"] + [
                        size[0] for size in product_sizes
                    ]
                ...
                # === Title Label ===
                product_title = Label(
                    product_frame, text="Product Details", font=("Garamond", 20)
                )
                product_title.place(x=50, y=120, width=1200)

                # === Label & Entry for Product ID ===
                id_label_1 = Label(
                    product_frame, text="Product ID", font=("Garamond", 15)
                )
                ...
                name_label_1 = Label(
                    product_frame, text="Product Name", font=("Garamond", 15)
                )
                ...
                size_label_1 = Label(
                    product_frame, text="Prodcut Size", font=("Garamond", 15)
                )
                ...
                price_label_1 = Label(
                    product_frame, text="Prodcut Price", font=("Garamond", 15)
                )
                ...
                scrollbar_y = Scrollbar(table_frame_p, orient=VERTICAL)
                scrollbar_x = Scrollbar(table_frame_p, orient=HORIZONTAL)
                ...
                product_frame.pack(
                    fill="both", expand=True
                )  # Ensure product_frame expands to fill main_frame
            ...
            # =====================================================================================================================
                ...
                def show_looms_data():
                    try:
                        self.mycursor.execute(
                            "SELECT Looms.Loom_No, Product.Size, Product.Name FROM Looms JOIN Product ON Looms.Product_Id = Product.Product_Id ORDER BY Loom_No"
                        )
                    ...
                    if loom_no_cb.get() == "-Select-":
                        ...
                        try:
                            ...
                            if product_id_row:
                                ...
                                self.mycursor.execute(
                                    sql_update_looms, (product_id_val, loom_no_cb_val)
                                )
                                self.mydb.commit()

                                messagebox.showinfo(
                                    "Success", "Loom data updated successfully."
                                )
                                ...
                                messagebox.showerror(
                                    "Oops",
                                    "Product size is not present, try adding it first.",
                                )
                ...
                # === Title label ===
                looms_title = Label(
                    looms_frame, text="Looms Detail", font=("Garamond", 20)
                )
                looms_title.place(x=50, y=120, width=1200)

                # === Label and combobox for Loom No ===
                loom_no_label = Label(
                    looms_frame, text="Loom No", font=("Garamond", 15)
                )
                ...
                product_id_label = Label(
                    looms_frame, text="Product Size", font=("Garamond", 15)
                )
                ...
                loom_sb_y = Scrollbar(loom_table_frame, orient=VERTICAL)
                loom_sb_x = Scrollbar(loom_table_frame, orient=HORIZONTAL)
                ...
                looms_frame.pack(
                    fill="both", expand=True
                )  # Ensure looms_frame expands to fill main_frame
            ...
            # =====================================================================================================================
                ...
                def save_worker_data():
                    ...
                    if worker_name.get() == "" or worker_surname.get() == "":
                        ...
                        messagebox.showerror(
                            "Oops!",
                            "You missed a spot. Please fill in the required textbox.",
                        )

                    # Check if the worker name or surname contains digits
                    elif (
                        not worker_name.get().isalpha()
                        or not worker_surname.get().isalpha()
                    ):
                        # If yes then display an error message
                        messagebox.showerror(
                            "Oops!", "Name and Surname should include alphabets only."
                        )
                        ...
                        try:
                            ...
                            sql_insert = (
                                "INSERT INTO workers(Name, Surname) VALUES (%s, %s)"
                            )
                            ...
                            messagebox.showinfo(
                                "Success", "Worker data saved successfully"
                            )
                ...
                def update_worker_data():
                    ...
                    if worker_id.get() == "-Select-":
                        ...
                        try:
                            ...
                            # Display a success message
                            messagebox.showinfo(
                                "Success", "Worker data updated successfully"
                            )
                ...
                def delete_worker_data():
                    if worker_id.get() == "-Select-":
                        messagebox.showerror(
                            "Oops!", "Please enter Worker ID before proceeding."
                        )
                        ...
                        try:
                            ...
                            if accept == True:
                                ...
                                messagebox.showinfo(
                                    "Deleted", "Worker data deleted successfully"
                                )
                ...
                def search_worker():
                    try:
                        if search_by.get() == "":
                            messagebox.showerror(
                                "Oops!", "Please choose an option from the Combobox"
                            )
                        ...
                        # If combobox has '-Select-' option then show error message
                            ...
                            if search_text.get() == "":
                                ...
                                messagebox.showerror(
                                    "Oops!", "Please type something to search for."
                                )
                ...
                search_frame.place(
                    x=350, y=10, width=700, height=100
                )  # Place the search_frame

                # === Label for Combobox ===
                label_for_combobox = Label(
                    search_frame, text="Search By", font=("Garamond", 12)
                )
                ...
                search_combobox.place(
                    x=100, y=10, width=180
                )  # Place the combobox within search_frame
                ...
                # === Title Label ===
                worker_title = Label(
                    workers_frame, text="Workers Details", font=("Garamond", 20)
                )
                worker_title.place(x=50, y=120, width=1200)

                # === Label & Entry for Worker ID ===
                id_label = Label(workers_frame, text="Worker ID", font=("Garamond", 15))
                ...
                name_label = Label(
                    workers_frame, text="Worker Name", font=("Garamond", 15)
                )
                ...
                surname_label = Label(
                    workers_frame, text="Worker Surname", font=("Garamond", 15)
                )
                ...
                workers_frame.pack(
                    fill="both", expand=True
                )  # Ensure workers_frame expands to fill main_frame
            ...
            # =====================================================================================================================
                ...
                def save_production_data():
                    if production.get() == "":
                        ...
                        messagebox.showerror(
                            "Oops!", "Production must be a numeric value."
                        )
                    else:
                        loom_no_var, worker_id_var, production_var = (
                            gather_production_data()
                        )
                        ...
                        try:
                            ...
                            if existing_record:
                                messagebox.showerror(
                                    "Oops!",
                                    "Production data for this loom number already exists for today.",
                                )
                            else:
                                ...
                                messagebox.showinfo(
                                    "Success",
                                    f"{today_day}'s Production has been saved successfully.",
                                )
                    ...
                    if production.get() == "":
                        ...
                        try:
                            ...
                            if num_records == 0:
                                messagebox.showerror(
                                    "No Data",
                                    f"No data found for Loom No {loom_no_var}. Please enter data for this loom first.",
                                )
                                ...
                                self.mycursor.execute(
                                    sql_update_production,
                                    (production_var, worker_id_var, loom_no_var),
                                )
                                self.mydb.commit()
                                messagebox.showinfo(
                                    "Success",
                                    f"Updated Loom no {loom_no_var}'s data for today.",
                                )
                ...
                production_title = Label(
                    production_frame, text="Production Details", font=("Garamond", 20)
                )
                production_title.place(x=50, y=120, width=1200)

                loom_label = Label(
                    production_frame, text="Loom No", font=("Garamond", 15)
                )
                ...
                worker_id_list = ["-Select-"] + [
                    worker[0] for worker in worker_ids
                ]  # Extract Worker_Id values from the result
                worker_label2 = Label(
                    production_frame, text="Worker ID", font=("Garamond", 15)
                )
                ...
                production_label = Label(
                    production_frame, text="Production", font=("Garamond", 15)
                )
                ...
                p_sb_y1 = Scrollbar(worker_table_frame2, orient=VERTICAL)
                p_sb_y1.pack(side=RIGHT, fill=Y)
                p_sb_x1 = Scrollbar(worker_table_frame2, orient=HORIZONTAL)
                ...
                p_sb_y = Scrollbar(production_table_frame, orient=VERTICAL)
                p_sb_y.pack(side=RIGHT, fill=Y)
                p_sb_x = Scrollbar(production_table_frame, orient=HORIZONTAL)
                ...
                production_frame.pack(
                    fill="both", expand=True
                )  # Ensure production_frame expands to fill main_frame
            ...
            # =====================================================================================================================
                ...
                def zoom_in_chart():
                    """Increase chart zoom level"""
                    display_report.zoom_level += 0.2  # Increase by 20%
                    if display_report.zoom_level > 3.0:  # Max zoom 300%
                        display_report.zoom_level = 3.0
                    display_report()  # Redraw chart with new zoom

                def zoom_out_chart():
                    """Decrease chart zoom level"""
                    display_report.zoom_level -= 0.2  # Decrease by 20%
                    if display_report.zoom_level < 0.4:  # Min zoom 40%
                        display_report.zoom_level = 0.4
                    display_report()  # Redraw chart with new zoom

                # Function for displaying the report
                def display_report():
                    # Store current zoom level
                    if not hasattr(display_report, "zoom_level"):
                        display_report.zoom_level = 1.0  # Default zoom level

                    # Store frame references to destroy previous charts
                    if not hasattr(display_report, "current_frame"):
                        display_report.current_frame = None
                    ...
                    from_date1 = parse_date_entry(from_date_report)
                    to_date1 = parse_date_entry(to_date_report)

                    # Placing the radiobuttons
                    rb1.place(x=840, y=15)
                    rb2.place(x=1000, y=15)

                    # Placing zoom buttons (appear after radio buttons)
                    zoom_in_btn = Button(
                        report_frame,
                        text="+",
                        font=("Garamond", 20, "bold"),
                        cursor="hand2",
                        command=zoom_in_chart,
                    )
                    zoom_in_btn.place(x=1150, y=15, height=30, width=50)
                    ...
                    zoom_out_btn = Button(
                        ...
                        font=("Garamond", 20, "bold"),
                        cursor="hand2",
                        command=zoom_out_chart,
                    )
                    zoom_out_btn.place(x=1210, y=15, height=30, width=50)

                    if display_report.current_frame is not None:
                        display_report.current_frame.destroy()
                    ...
                    if var.get() == 1:
                        try:
                            ...
                            self.mycursor.execute(
                                "SELECT Date, SUM(Production) FROM Production WHERE Date BETWEEN %s AND %s GROUP BY Date",
                                (from_date1, to_date1),
                            )
                            ...
                            production_values = [
                                float(row[1]) for row in production_data
                            ]

                            # 3. Create the line graph within the report_frame
                            # Apply zoom level to figure size
                            base_width = 15.8
                            base_height = 8.5
                            fig, ax = plt.subplots(
                                figsize=(
                                    base_width * display_report.zoom_level,
                                    base_height * display_report.zoom_level,
                                )
                            )
                            ...
                            ax.set_title(
                                f"Production Report from {from_date1} to {to_date1}"
                            )
                            fig.tight_layout()
                            ...
                            f.place(x=5, y=60)
                            display_report.current_frame = (
                                f  # Store current frame reference
                            )
                            self.mydb.commit()
                        ...
                        try:
                            ...
                            ax1.set_title(
                                f"Salaries paid between {from_date1} to {to_date1}"
                            )
                            ...
                            u.place(x=5, y=60)
                            display_report.current_frame = (
                                u  # Store current frame reference
                            )
                            self.mydb.commit()
                ...
                date_label_from = Label(
                    report_frame, text="From", font=("Garamond", 20)
                )
                date_label_from.place(x=10, y=10)
                date_label_to = Label(report_frame, text="To", font=("Garamond", 20))
                date_label_to.place(x=350, y=10)

                # Date widget from ttkbootstrap
                report_from_date = tb.DateEntry(
                    report_frame, bootstyle="danger", dateformat="%m/%d/%y"
                )
        ...
        # === Frame1 for Menu ===
        left_menu_frame = Frame(
            self.root, relief=RIDGE, bd=2
        )  # add bd = 2 if you want border to highlight
        left_menu_frame.place(x=0, y=100, width=300, height=920)

        # ===Menu Label inside the Frame===
        menu_label = Label(
            left_menu_frame, text="Menu", font=("Garamond", 30), height=5
        ).pack(
            side=TOP, fill=X
        )  # Packing the 'Menu' label inside the frame
        ...
        )  # Packing the 'Production' button inside the frame
        ...
        home_button = Button(
            ...
            side=TOP, fill=X, padx=20, pady=10
        )  # Packing the 'Home' button inside the frame
        ...
        worker_button = Button(
            ...
            side=TOP, fill=X, padx=20, pady=10
        )  # Packing the 'Worker' button inside the frame
        ...
        product_button = Button(
            ...
            side=TOP, fill=X, padx=20, pady=10
        )  # Packing the 'Product' button inside the frame
        ...
        looms_button = Button(
            ...
            side=TOP, fill=X, padx=20, pady=10
        )  # Packing the 'Looms' button inside the frame
        ...
        exit_button = Button(
            ...
            side=TOP, fill=X, padx=20, pady=10
        )  # Packing the 'Exit' button inside the frame
        ...
        # --==--==NEW:--==--==
        image_path = resource_path("images/android-chrome-512x512.png")
    ...
    def redirect_to_homepage(self):
        ...
        try:
            ...
            if getattr(sys, "frozen", False):
                # Running as EXE - launch Gajul_Fabrics.exe
                main_path = os.path.join(
                    os.path.dirname(sys.executable), "Gajul_Fabrics.exe"
                )
                subprocess.Popen([main_path])
            else:
                # Running as Python script - launch main.py
                subprocess.Popen(["python", "main.py"])
            self.root.quit()
        except Exception as ex:
...
if __name__ == "__main__":
    ...
    root = tb.Window(
        themename="superhero"
    )  # Feel free to customize the themes to suit your preferences and visual style.
